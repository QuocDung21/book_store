...
export const get_comments = createAsyncThunk(
  ...
  async (_, { rejectWithValue, fulfillWithValue }) => {
    try {
      const { data } = await api.get("/home/get-all-reviews", {
        withCredentials: true,
      });
      return fulfillWithValue(data);
    } catch (error) {
...
export const check_comment = createAsyncThunk(
  "auth/check_comment",
  async (obj, { rejectWithValue, fulfillWithValue }) => {
    try {
      const { data } = await api.post("/home/check-review", obj, {
        withCredentials: true,
      });
      return fulfillWithValue(data);
    } catch (error) {
...
export const authReducer = createSlice({
  ...
  extraReducers: {
    ...
    [get_auth.pending]: (state, _) => {
      state.loader = true;
    },
    [get_auth.fulfilled]: (state, { payload }) => {
      state.loader = false;
      state.users = payload.userInfo;
      state.successMessage = payload.message;
    },

    [get_comments.pending]: (state, _) => {
      state.loader = true;
    },
    [get_comments.rejected]: (state, { payload }) => {
      state.loader = false;
      state.errorMessage = payload.error;
    },
    ...
    [check_comment.pending]: (state, _) => {
      state.loader = true;
    },
    [check_comment.rejected]: (state, { payload }) => {
      state.loader = false;
      state.errorMessage = payload.error;
    },
    [check_comment.fulfilled]: (state, { payload }) => {
      state.loader = false;
      state.successMessage = payload.message;
    },
  },
