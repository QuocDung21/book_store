...
const {paymentStatus, payment} = require("../../utils/default");
...
class orderController {
        ...
        try {
            ...
            if (order.payment_status === "Chưa thanh toán") {
                await customerOrder.findByIdAndUpdate(id, {
                    delivery_status: "Đang Chờ Thanh Toán",
                });
    ...
    seller_order_status_update = async (req, res) => {
        ...
        const {status} = req.body;
        try {
            const data = await authOrderModel.findById(orderId);
            await authOrderModel.findByIdAndUpdate(orderId, {
                delivery_status: status,
            });
            await customerOrder.findByIdAndUpdate(data.orderId, {
                delivery_status: status,
            });
            responseReturn(res, 200, {message: "order status change success"});
        } catch (error) {
    ...
    order_confirm = async (req, res) => {
        ...
        try {
            ...
            order.products.map(async (product) => {
                await decreaseProductQuantity(product._id, parseInt(product.quantity));
            });
            await customerOrder.findByIdAndUpdate(orderId, {
...
const decreaseProductQuantity = async (productId, quantity) => {
    try {
        ...
        console.error("Error in decreasing product quantity:", error.message);
    }
};
